...
DateTime dateTimeAlt = DateTime(14, 6, 21, DateTime::MONDAY, 7, 54, 30); // Kopie der Zeit um die Aktualisierung zu plausibilisieren
...
//                   -----   ----- ----- ----- ----- ----- ----- ----- -----
int StundeX[20]={ 
  7, 8, 9, 9,10,10,10,11,11,12,12,13,13,14,14,15,15,15,15,16};          // Daten für Unterrichtsstunden Stunde
int MinuteX[20]={
  55, 0,40,55, 0,45,50,35,45,30,35,20,25,10,15,00,05,50,55,40};          // Daten für Unterrichtsstunden Minute
//               ----- -------- 
//               |     |  ----- ----- ----- ----- ----- ----- ----- -----
//               |     |  |     |     |     |     |     |     |     |
//               |     |  |     |     |     |     |     |     |     7. Pause
...
//                        ----- ----- ----- ----- ----- ----- ----- ----- ----- -----
int FerienTag14[20]    = { 
  1, 6, 3, 7,14,25, 1, 2,29,30, 9,20,31,12, 8, 9,27,31,22,31}; // Feriendaten für 2014 Tag 
int FerienMonat14[20]  = { 
  1, 1, 3, 3, 4, 4, 5, 5, 5, 5, 6, 6, 7, 9,10,10,10,10,12,12}; // Feriendaten für 2014 Monat
int FerienTag15[20]    = { 
  1, 6,16,20,30,10, 1, 2,14,15,25, 5,30,11, 3, 4, 2, 6,23,31}; // Feriendaten für 2015 Tag
int FerienMonat15[20]  = { 
  1, 1, 2, 2, 3, 4, 5, 5, 5, 5, 5, 6, 7, 9,10,10,11,11,12,12}; // Feriendaten für 2015 Monat
int FerienTag16[20]    = { 
  1, 8, 8,12,25, 1,30, 1, 5, 6,16,27,28, 9, 2, 3,31, 4,23,31}; // Feriendaten für 2016 Tag
int FerienMonat16[20]  = { 
  1, 1, 2, 2, 3, 4, 4, 5, 5, 5, 5, 5, 7, 9,10,10,10,11,12,12}; // Feriendaten für 2016 Monat
int FerienTag17[20]    = { 
  1, 6,27, 3,10,21,30, 1,25,26, 5,16,27, 8, 2, 3,30, 3,25,31}; // Feriendaten für 2017 Tag
int FerienMonat17[20]  = { 
  1, 1, 2, 3, 4, 4, 4, 5, 5, 5, 6, 6, 7, 9,10,10,10,11,12,12}; // Feriendaten für 2017 Monat
int FerienTag18[20]    = { 
  1, 7,12,16,26, 6,30, 1,10,11,21,02,26, 8, 3, 4, 1, 2,22,31}; // Feriendaten für 2018 Tag
int FerienMonat18[20]  = { 
  1, 1, 2, 2, 3, 4, 4, 5, 5, 5, 5, 6, 7, 9,10,10,11,11,12,12}; // Feriendaten für 2018 Monat
int FerienTag19[20]    = { 
  1, 6, 4, 8,15,26, 1, 2,30,31,11,21,29,10, 3, 4, 1, 2,21,31}; // Feriendaten für 2019 Tag
int FerienMonat19[20]  = { 
  1, 1, 3, 3, 4, 4, 5, 5, 5, 5, 6, 6, 7, 9,10,10,11,11,12,12}; // Feriendaten für 2019 Monat
//------------------------------------------------------------------------------------------------------------------------------
...
int DebugAktiv;                                // Für Testzwecke, z.B. die Plausibilisierung umgehen
int ErsterEmpfang;                             // Empfang erstmals nach einem Reset oder Spannungsausfall
int unplausibel;                               // Letzter Zeitstempel war nicht plausibel, Empfang neu starten
...
void setup()
{
  ...
  Serial.print("Initialisierung beginn ... ");
  ...
  ErsterEmpfang=1;  // Erstmaligen Empfang nach Reset oder Spannungsausfall aktivieren
  ...
  DCF77EmpfangAktiv = 0;      // erst mal initialisieren bis das Datum und die Uhrzeit geprüft wird
...
void loop()
{
  ...
  DCF77ZeitHolen();                // Uhrzeit und Datum vom RTC holen
...
void printClock(void)                  // Ausgabe der Zeit und Datum vom ELV-CDF77 auf dem Terminal und Kopieren der Daten in globale Variablen
{
  ...
  sprintf(clockString, "%02u:%02u:%02u am %02u.%02u.%02u ", Stunde, Minute, Sekunde, Tag, Monat, Jahr);
...
void DCF77(void)                         // Prüfung ob Datum und Uhrzeit gültig sind oder eine tägliche Aktualisierung fällig ist
{
  ...
  if ( ( (Jahr < 14) || (Jahr > 20) || (Monat > 12) || (Tag > 31) || (Stunde > 23) || (Minute > 59) || ((Stunde == 03) && (Minute == 30)) || unplausibel) && (DCF77EmpfangAktiv == 0)  )
  {
...
void DCF77ZeitHolen(void)
{
  RTC_DCF.getDateTime(&dateTime);  // Uhrzeit und Datum vom RTC holen
  if(ErsterEmpfang || DebugAktiv)  // Zu Testzwecken oder nach einem Reset oder Stromausfall nicht plausibilieren
  {
...
//      }
    ...
    {
      //      if(!(dateTime.getWeekday() == Wochentag || dateTime.getWeekday() == Wochentag + 1)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Wochentag ");
      //      }
    ...
    if (!(dateTime.getYear() == Jahr || dateTime.getYear() == Jahr + 1)) 
    {
      //      unplausibel = 1;
      //      Serial.print("Jahr ");
    }
    ...
    if (Monat == 12)
    {
      //      if(!(dateTime.getMonth() == 12 || dateTime.getMonth() == 12)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Monat ");
      //      }
    ...
    {
      //      if(!(dateTime.getMonth() == Monat || dateTime.getMonth() == Monat + 1)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Monat ");
      //      }
    ...
    if (Tag == 28)
    {
      //      if(!(dateTime.getDay() == 28 || dateTime.getDay() == 0)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Tag ");
      //      }
    ...
    if (Tag == 29)
    {
      //      if(!(dateTime.getDay() == 29 || dateTime.getDay() == 0)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Tag ");
      //      }
    ...
    if (Tag == 30)
    {
      //      if(!(dateTime.getDay() == 30 || dateTime.getDay() == 0)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Tag ");
      //      }
    ...
    if (Tag == 31)
    {
      //      if(!(dateTime.getDay() == 31 || dateTime.getDay() == 0)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Tag ");
      //      }
    ...
    if (Tag < 28)
    {
      //      if(!(dateTime.getDay() == Tag || dateTime.getDay() == Tag + 1)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Tag ");
      //      }
    ...
    if (Stunde == 23)
    {
      //      if (!(dateTime.getHour() == 23 || dateTime.getHour() == 0)) 
      //      {
      //        unplausibel = 1;
      //        Serial.print("Stunde ");
      //      }
    ...
    {
      //      if(!(dateTime.getHour() == Stunde || dateTime.getHour() == Stunde + 1 || dateTime.getHour() == Stunde + 2 || dateTime.getHour() == Stunde - 1))
      //      {
      //        unplausibel = 1;
      //        Serial.print("Stunde ");
      //      }
    ...
    if ((Minute + 5) > 59)
    {
      if(!(dateTime.getMinute() > (Minute - 5) || dateTime.getMinute() < (Minute - 54)))
      {
        //        unplausibel = 1;
        //        Serial.print("Minute ");
      }
    ...
    {
      //      if(!(dateTime.getMinute() > (Minute + 54) || dateTime.getMinute() < (Minute + 5)))
      //      {
      //        unplausibel = 1;
      //        Serial.print("Minute ");
      //      }
    ...
    {
      //      if(!(dateTime.getMinute() > (Minute - 5) && dateTime.getMinute() < (Minute +5)))
      //      {
      //        unplausibel = 1;
      //        Serial.print("Minute ");
      //      }
    ...
    {
      Serial.print("Zeitstempel unplausibel ");
      Sekunde++;
      if (Sekunde == 60)
      {
        Sekunde = 0;
