...
byte DCF77EmpfangAktiv,DCF77EmpfangErfolgreich; // Hilfsvariablen für die tägliche Aktualisierung der Zeit und des Datums über DCF77
byte DebugAktiv;                                // Für Testzwecke, z.B. die Plausibilisierung umgehen
byte ErsterEmpfang;                             // Empfang erstmals nach einem Reset oder Spannungsausfall
byte unplausibel;                               // Letzter Zeitstempel war nicht plausibel, Empfang neu starten
...
void DCF77(void)                         // Prüfung ob Datum und Uhrzeit gültig sind oder eine tägliche Aktualisierung fällig ist
{
  ...
  if (ErsterEmpfang) Serial.print("Initial ");         // Solange der Empfang aktiv ist soll dies auf dem Terminal ausgegeben werden
  else if (DCF77EmpfangAktiv) Serial.print("Empfang ");       // Solange der Empfang aktiv ist soll dies auf dem Terminal ausgegeben werden
  if (ErsterEmpfang || DCF77EmpfangAktiv)                                // Wenn Empfang
  {
...
void DCF77ZeitHolen(void)
{
  ...
  {
    ZeitKopieren();
    if(DCF77EmpfangErfolgreich) ErsterEmpfang = 0;
  }
  else
  {
    unplausibel = 0;
